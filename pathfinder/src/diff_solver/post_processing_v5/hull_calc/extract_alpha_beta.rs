...
use crate::diff_solver::post_processing_v5::{bt, Handlers, hull_calc, SolvedSocMeta, utils, AnalysisMode};
        ...
        P: PPFactory,
{
    ...
    // Upper limit of how many inner paths we are to extract
    ...
    let path_probabilities = match path_probabilities {
        ...
        Err(probs) => {
            ...
            // We didn't extract all, notify how many we skipped.
            // TODO, do as part of above mentioned missing notification instead.
            result_builder2.paths_skipped = Some(sum_inner_paths - upper_lim);
            probs
        }
    };


    let path_probabilities: BTreeMap<usize, usize> = match mode{
        Linear => {
            // we need to double the infl's
            path_probabilities.into_iter()
                .map(|(infl, count)| (infl *2, count))
                .collect()
        },
        Differential => {
            path_probabilities.into_iter()
